...
\begin{frame}{Insights \& Takeaways}
  \small
  \begin{block}{Wrapper-based observability is practical and adaptable}
    Zero agent-side changes apart from some action state naming. Adpatable and transparent for LLM agents.
  \end{block}
  \vspace{0.2em}
  \begin{block}{Recall $>$ Precision is the right trade-off}
    A missed violation is worse than a false alarm -- prefer escalation over silent permit.
  \end{block}
  \vspace{0.2em}
  \begin{block}{Trust decay enables lightweight adaptation}
    Per-fingerprint scores adjust from operator decisions; no model fine-tuning needed.
  \end{block}
  \vspace{0.2em}
  \begin{block}{Taint detects behavioral compromise, not just rule violations}
    A malicious system prompt raised risk on safe sinks immediately --
    caught purely from data flow, without LLM-specific knowledge.
  \end{block}
\end{frame}
